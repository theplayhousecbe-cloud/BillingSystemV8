  ...
  const handleSubmit = () => {
    ...
    sessionsApi.create({
      ...
      pricing: {
        ...
        custom: false,
        selectAllMenu: false,
        memberRates: Array(adults + kids + kidsAbove10).fill(149).map((_, i) => {
          if (i < adults) return 149;
          if (i < adults + kids) return 99;
          return 149; // kids above 10
        }),
      },
    });
      ...
      <div className="space-y-5">
        ...
        <div className="glass rounded-2xl p-4 space-y-3">
          ...
          <div className="flex gap-6">
            ...
            <div className="flex flex-col gap-1">
              <span className="text-xs text-muted-foreground">Kids &lt;10</span>
              <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col gap-1">
              <span className="text-xs text-muted-foreground">Kids &gt;10</span>
              <div className="flex items-center gap-2">
                <button onClick={() => setKidsAbove10(Math.max(0, kidsAbove10 - 1))} className="glass h-8 w-8 rounded-full text-lg font-bold flex items-center justify-center">−</button>
                <span className="w-6 text-center font-semibold">{kidsAbove10}</span>
                <button onClick={() => setKidsAbove10(kidsAbove10 + 1)} className="glass h-8 w-8 rounded-full text-lg font-bold flex items-center justify-center">+</button>
              </div>
            </div>
          </div>
        </div>
